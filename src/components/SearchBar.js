    ...
    return (
        <div className="relative">
            <nav className="flex items-center justify-between bg-orange-500 p-4 shadow-md">
                ...
                <button className="bg-white text-blue-500 p-2 w-40 rounded-md transition" onClick={() => handleCityClick(query)}>
                    <b>Search</b>
                </button>
            </nav>
            {dropdownVisible && (
                <div ref={dropdownRef} className="absolute bg-white border border-gray-300 rounded-md mt-1 w-96 z-10">
            ...
            <div className="mt-4">
                {hotels.length > 0 ? (
                    ...
                    <p className="text-gray-700">No hotels found for the selected city.</p>
                )}
            </div>
        </div>
    );
